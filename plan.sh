...
_parse_plan () {
    ...
    p.)
        ...
        ;;
    c.)
        _parse_plan "$pattern" | cut -d'|' -f1 >$TMP/children_all
    ...
    *) 
    ...
    out=$(echo "$hash" | $DATA ..append @$NAME_KEY)
    $CORE return-parse "$out" "$h" 
}
...
pursuit)
    _handle_plan_pursuit "$@"
    _open $hash
    pursuit_hash=$(_parse_plan "_.$pursuit") || pursuit_hash=$(\
        _parse_plan "n.$pursuit")
